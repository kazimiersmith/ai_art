...
pd <- import('pandas')
panel <- pd$read_pickle(paste0(data, '/freq_data.pkl'))
panel_both <- pd$read_pickle(paste0(data, '/freq_data_both.pkl'))
...
# Posts
reg1 <- lm(posts_organic ~ 0 + posts_ai, data = panel)
reg2 <- lm(posts_per_author_organic ~ 0 + posts_per_author_ai, data = panel)
reg3 <- lm(authors_organic ~ 0 + authors_ai, data = panel)
reg4 <- lm(post_score_organic ~ 0 + post_score_ai, data = panel)

# Comments
reg5 <- lm(comments_organic ~ 0 + comments_ai, data = panel)
reg6 <- lm(comments_per_author_organic ~ 0 + comments_per_author_ai, data = panel)
reg7 <- lm(commenters_organic ~ 0 + commenters_ai, data = panel)
reg8 <- lm(comments_per_post_organic ~ 0 + comments_per_post_ai, data = panel)

reg9 <- lm(comments_organic ~ 0 + comments_ai, data = panel_both)
reg10 <- lm(comments_per_author_organic ~ 0 + comments_per_author_ai, data = panel_both)
reg11 <- lm(commenters_organic ~ 0 + commenters_ai, data = panel_both)
reg12 <- lm(comments_per_post_organic ~ 0 + comments_per_post_ai, data = panel_both)
